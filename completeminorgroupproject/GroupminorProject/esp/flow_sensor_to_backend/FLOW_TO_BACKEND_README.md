# Flow Sensor → Express Backend → AIML

## Data flow

1. **ESP32** reads sensors every 1 second, sends **raw** data to `POST /api/sensors`
2. **Main backend** buffers 3 samples, computes **mean** per parameter (1–3s window)
3. **ML model** runs on the simplified (grouped) data
4. Predictions and sensor state broadcast via WebSocket
